...
function validateMilliseconds(name, options, mandatory, minValue) {
  const value = options[name];
  if (mandatory && (value === undefined || value === null)) throw new ConfigurationError(`${name} is a required option`);
  if (value === undefined || value === null) return;
  if (typeof value !== 'number') throw new ConfigurationError(`The supplied ${name} must be at number`);
  if (value < minValue) throw new ConfigurationError(`The supplied ${name} must be at least ${minValue}ms`);
  return value;
}
