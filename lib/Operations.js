...
const { XPoolError } = require('./Errors');

const DEFAULT_MESSAGES = {
  notice: ({ contextId, text }) => `[${contextId}] ${text}`,
  failed: ({ contextId, err }) => `[${contextId}] ${err.message}`,
};
...
class XPoolOperation {
  ...
  start() {
    ...
    this._emitEvent(code, { code, message });
  }
  ...
  succeeded(duration) {
    ...
    this._emitEvent(code, { code, message, duration });
    return this;
  }
  ...
  failed(err) {
    ...
    const message = this._messages.failed({ contextId, err });
    this._emitError(code, { code, message, err });
    return this;
  }
  ...
  _emitEvent(code, payload) {
    this._pool.emit(code, payload) | this._pool.emit(XPoolEvent, payload) | this._pool.emit('X-POOL_EVENT', payload);
  }

  _emitError(code, payload) {
    this._pool.emit(code, payload) | this._pool.emit(XPoolError, payload) | this._pool.emit(XPoolEvent, payload) | this._pool.emit('X-POOL_EVENT', payload);
  }
}
