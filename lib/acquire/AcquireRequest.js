...
class AcquiredRequest {
  ...
  constructor(id, handler, validate, backoff) {
    this.#id = id;
  ...
  shouldValidate({ releasedAt }) {
    if (this.#validate === 'ALWAYS') return true;
    if (this.#validate === 'CREATE' && !Boolean(releasedAt)) return true;
    if (this.#validate === 'IDLE' && Boolean(releasedAt)) return true;
    return false;
  }
