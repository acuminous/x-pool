...
const ReleaseRequest = require('./requests/ReleaseRequest');
const DestroyRequest = require('./requests/DestroyRequest');
...
class XPool extends SafeEventEmitter {
  ...
  async start() {
    ...
    await this.#startRequest.run(async () => {
  ...
  async stop() {
    ...
    await this.#stopRequest.run(async () => {
  ...
  async release(resource) {
    return new ReleaseRequest().run(async () => {
  ...
  async destroy(resource) {
    return new DestroyRequest().run(async () => {
      const bay = this.#repository.locate(resource);
      await bay.destroy();
      this.#checkQueue();
      this.#checkReplenish();
