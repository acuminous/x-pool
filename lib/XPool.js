...
class XPool extends SafeEventEmitter {
  ...
  #addEventListeners() {
    this.#eventListeners.set(XPoolEvents.RESOURCE_RELEASED, () => this.#checkQueue());
    this.#eventListeners.set(XPoolEvents.RESOURCE_DESTROYED, () => this.#checkQueue());
    this.#eventListeners.forEach((value, key) => this.on(key, value));
  }

  #replaceEventListeners() {
    this.#eventListeners.forEach((value, key) => this.off(key, value));
    this.#eventListeners.set(XPoolEvents.RESOURCE_RELEASED, () => this.#repository.cull(this.#stopOperation));
    this.#eventListeners.set(XPoolEvents.RESOURCE_DESTROYED, () => this.#checkStop());
  }
}
