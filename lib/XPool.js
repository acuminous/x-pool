...
class XPool extends SafeEventEmitter {
  ...
  constructor(config) {
    super((error) => this.#disable(error));
  ...
  async start() {
    if (this.#panic) throw this.#createPoolDisabledError();
    if (this.#startOperation.initiated) throw this.#createPoolAlreadyStartedError();
    if (this.#stopOperation.initiated) throw this.#createPoolAlreadyStoppedError();
    ...
    await this.#startOperation.run(async () => {
      ...
      const operations = this.#initialiseBays(this.#repository.deficit, timeLimit);
  ...
  async acquire(requestId = shortId()) {
    if (this.#panic) throw this.#createPoolDisabledError();
    if (this.#stopOperation.initiated) throw this.#createPoolStoppedError();
  ...
  async release(resource) {
    if (this.#panic) throw this.#createPoolDisabledError();
    if (!this.#startOperation.initiated) throw this.#createPoolNotStartedError();
  ...
  async #initialiseBays(n, timeLimit) {
    return PromiseUtils.times(n, () => {
  ...
  #replenish() {
    ...
    setImmediate(async () => {
      debug('Replenishing pool with %d resources', 1);
      const timeLimit = new TimeLimitless('replenish pool');
      const operations = this.#initialiseBays(1, timeLimit);
      await timeLimit.restrict(operations);
    });
  }
  ...
  #addEventListeners() {
    ...
    this.#eventListeners.add(XPoolEvents.RESOURCE_DESTROYED, () => unscope(() => this.#replenish()));
    this.#eventListeners.add(XPoolEvents.RESOURCE_ACQUIRED, () => unscope(() => this.#replenish()));
  ...
  #createPoolDisabledError() {
    return new Error('The pool has been disabled', { cause: this.#panic });
  }

  #createPoolAlreadyStartedError() {
    return new Error('The pool has already been started');
  }

  #createPoolAlreadyStoppedError() {
    return new Error('The pool has already been stopped');
  }

  #createPoolNotStartedError() {
    return new Error('The pool has not been started');
  }

  #createPoolStoppedError() {
    return new Error('The pool has been stopped');
  }
}
