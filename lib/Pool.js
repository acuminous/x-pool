...
const StartRequest = require('./request/StartRequest');
const StopRequest = require('./request/StopRequest');
...
class Pool extends SafeEventEmitter {
  ...
  async start() {
    if (this.#startRequest) throw new Error('The pool has already been started');
    if (this.#stopRequest) throw new Error('The pool has already been stopped');

    debug(`Starting pool with ${this.#config.minSize} resources within ${this.#config.startTimeout.toLocaleString()}ms`);
    this.#startRequest = new StartRequest().initiate();
  ...
  async stop() {
    if (this.#startRequest) await this.#startRequest.wait();
    if (this.#stopRequest) return this.#stopRequest.wait();
    ...
    this.#stopRequest = new StopRequest().initiate();
  ...
  async acquire(requestId = randomUUID()) {
    if (this.#stopRequest) throw new Error('The pool has been stopped');
  ...
  #checkRepository() {
    if (this.#stopRequest) this.#repository.cull();
  }
