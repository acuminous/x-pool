...
module.exports = class Pool extends EventEmitter {
  ...
  async shutdown() {
    ...
    return runInContext(async () => {
      ...
      await this._destroyIdleResources();
      await stopped;
    });
  }
  ...
  async _checkShutdown() {
    if (this._stopping) await this._destroyIdleResources();
  }

  async _destroyIdleResources() {
    const destroyResources = this._store.getAllIdleResources().map((resource) => this._factory.destroy(resource));
    await Promise.all(destroyResources);
    this.evictBadResources();
    if (this._store.isEmpty()) this._stopping.resolve();
  }
