...
module.exports = class Pool extends EventEmitter {
  ...
  evictBadResources() {
    return runInContext(() => {
      debug('[%d] Evicting %d bad resources', getContextId(), this._resourceStore.bad);
  ...
  async _destroySpareResources() {
    debug('[%d] Destroying %d spare resources', getContextId(), this._resourceStore.spare);
    const destroyResources = new Array(this._resourceStore.spare).fill().map(async () => {
  ...
  async _batchAquire(task) {
    const batchSize = Math.max(this._resourceStore.minSize - this._resourceStore.size, 0);
