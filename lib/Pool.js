...
class Pool extends SafeEventEmitter {
  ...
  async #checkPool() {
    ...
    if (this.#repository.deficit > 0) await this.#refillPool();
  }

  async #refillPool() {
    const timeLimit = new TimeLimitless('refill pool');
    const operations = this.#initialiseBays(timeLimit);
    await timeLimit.restrict(operations);
  }
  ...
  #createQueue() {
    const { maxQueueSize, backoff } = this.#config;
    return new Queue({ maxQueueSize, backoff });
  }
  ...
  #createRepository() {
    ...
    const repository = new Repository({ minPoolSize, maxPoolSize, minIdleResources }, commandFactory);
