...
module.exports = class Pool extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this._resourceStore = new ResourceStore({ maxSize: options.maxSize, minSize: options.minSize });
    this._acquireQueue = [];
  }
  ...
  release(resource) {
    return runInContext(() => {
      ...
      this._resourceStore.releaseAcquiredResource(resource);
  ...
  evictBadResources() {
    return runInContext(() => {
      debug('[%d] Evicting %d bad resources', getContextId(), this._resourceStore.stats().bad);
      this._resourceStore.evictBadResources();
    });
  }

  stats() {
    return this._resourceStore.stats();
  }
  ...
  async _checkShutdown() {
    ...
    if (this._resourceStore.isEmpty()) this._completeShutdown();
  }

  async _destroySpareResources() {
    const spare = this._countSpareResources();
    debug('[%d] Destroying %d spare resources', getContextId(), spare);
    const destroyResources = new Array(spare).fill().map(() => {
      const resource = this._resourceStore.getIdleResource();
      return this._factory.destroy(resource);
    });
    await Promise.all(destroyResources);
  }

  _countSpareResources() {
    const { idle, pending } = this.stats();
    return Math.max(0, idle - pending);
  }

  _createAcquireTask() {
    const fn = (task) => new Promise((resolve, reject) => {
      this._resourceStore.registerAcquire();
    ...
    const onLateResolve = (resource) => {
      ...
      this._resourceStore.addIdleResource(resource);
    };
  ...
  async _batchAquire(task) {
    const acquireResources = this._resourceStore.getEmptyBatch().map(async () => {
      let resource;
  ...
  async _acquireResource(task) {
    ...
    if (!task.isAborted()) this._resourceStore.addAcquiredResource(resource);
    return resource;
  }

  async _obtainValidResource() {
    const resource = this._resourceStore.getIdleResource() || await this._createResource();
