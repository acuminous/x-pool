...
class Pool extends SafeEventEmitter {
  ...
  async start() {
    ...
    const initialiseBerths = PromiseUtils.times(this.#config.minSize, () => {
      return this.#enqueue(randomUUID(), limit, (request) => this.#handleInitialise(request));
    });

    try {
      await limit.restrict(initialiseBerths);
    } finally {
  ...
  async stop() {
    ...
    const stopGracefully = new Promise(async (resolve) => {
    ...
    await limit.restrict(stopGracefully);
  }
