...
module.exports = class Pool extends EventEmitter {
  ...
  destroy(resource) {
    ...
    // Should the resource be removed after it has been successfully destroyed? Otherwise the db connections could grow
    this._store.removeAcquiredResource(resource);
    this._backgroundDestroy(resource);
    this._checkAcquireQueue(); // TODO Consider whether this should be here, since it is before the connection was destroyed
  }
