...
class Pool extends SafeEventEmitter {
  ...
  async start() {
    if (this.#startLatch) throw new Error('The pool has already been started');
    if (this.#stopLatch) throw new Error('The pool has been stopped');

    debug(`Starting pool within ${this.#config.startTimeout}`);
    this.#startLatch = new AsyncLatch();
  ...
  async stop() {
    if (this.#stopLatch) return this.#stopLatch.block();

    debug(`Stopping pool within ${this.#config.stopTimeout}ms`);
    this.#stopLatch = new AsyncLatch();
