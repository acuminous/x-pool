...
class Repository extends EventEmitter {
  ...
  stats() {
    const { initialising, idle, acquired, doomed, timedout, zombie } = this.#stores;
    return {
      size: this.size,
  ...
  extend() {
    ...
    const bay = new ResourceBay(this.#stores, this.#commandFactory, { validate });
  ...
  cull() {
    ...
    const doomed = this.#stores.idle.map((bay) => bay.reserve(this.#stopRequest).skipValidation());
