...
class Repository extends EventEmitter {
  ...
  async cull(stopOperation) {
    const idle = this.#stores.idle.map((bay) => bay.reserve(stopOperation).skipValidation());
    debug(`Culling ${idle.length.toLocaleString()} idle resources`);

    const destroyIdleResources = idle.map((bay) => bay.destroy().catch(() => {}));
