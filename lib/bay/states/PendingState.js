...
class PendingState extends BaseState {
  ...
  constructor(ward, createResource) {
    super(ward);
  ...
  #onCreateSuccess({ resource }) {
    ...
    this._bay._assign(resource);
    this._bay._toReady();
    this.notify(BayEvents.RESOURCE_CREATED);
  }

  #onCreateError({ error }) {
    debug(`Error provisioning ${this._bay.state} bay [${this._bay.shortId}]`, error);
    this.notify(BayEvents.RESOURCE_CREATION_ERROR, { error });
  ...
  #onCreateTimeout({ timeout }) {
    ...
    this.notify(BayEvents.RESOURCE_CREATION_TIMEOUT, { timeout });
    this._bay.segregate();
  }
  ...
  #onBelatedCreateError = ({ error }) => {
    ...
    this.notify(BayEvents.RESOURCE_CREATION_ERROR, { timeout, error });
    this._bay._toDestroyed();
  }
}
