...
class DoomedState extends BaseState {
  ...
  #onDestroySuccess() {
    ...
    this._notify(XPoolEvents.RESOURCE_DESTROYED);
  }

  #onDestroyError({ error }) {
    debug(`Error destroying ${this.name} bay`, error);
    this._notify(XPoolEvents.RESOURCE_DESTRUCTION_ERROR, { error });
  ...
  #onDestroyTimeout({ timeout }) {
    ...
    this._notify(XPoolEvents.RESOURCE_DESTRUCTION_TIMEOUT, { timeout });
    this._bay.segregate();
  }
