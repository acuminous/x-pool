...
class AcquiredState extends BaseState {
  ...
  skipReset() {
    debug(`Skipped reset for resource in ${this.name} bay [${this._bay.shortId}]`);
    this._stateMachine.toReset();
  }

  async reset(resource) {
    debug(`Resetting resource in ${this.name} bay [${this._bay.shortId}]`);
  ...
  #onResetSuccess() {
    debug(`Successfully reset resource in ${this.name} bay [${this._bay.shortId}]`);
    this._stateMachine.toReset();
    this.notify(XPoolEvents.RESOURCE_RESET);
  }

  #onResetError({ error }) {
    debug(`Error resetting resource in ${this.name} bay [${this._bay.shortId}]`, error);
    this.notify(XPoolEvents.RESOURCE_RESET_ERROR, { error });
    this.destroy();
  }

  #onResetTimeout({ timeout }) {
    debug(`Timed out after ${timeout.toLocaleString()}ms resetting resource in ${this.name} bay [${this._bay.shortId}]`);
    this.notify(XPoolEvents.RESOURCE_RESET_TIMEOUT, { timeout });
    this.#segregate();
  }

  #onBelatedResetSuccess() {
    debug(`Belated success reseting resource in ${this.name} bay [${this._bay.shortId}]`);
    this.notify(XPoolEvents.RESOURCE_RESET);
    this._bay.destroy();
  }

  #onBelatedResetError = ({ error }) => {
    debug(`Belated error reseting resource in ${this.name} bay [${this._bay.shortId}]`, error);
    this.notify(XPoolEvents.RESOURCE_RESET_ERROR, { error });
    this._bay.destroy();
  };
