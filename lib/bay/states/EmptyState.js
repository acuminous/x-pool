...
class EmptyState extends BaseState {
  ...
  abandon() {
    ...
    this._notify(XPoolEvents.RESOURCE_CREATION_ABANDONED);
  }
  ...
  #onCreateSuccess({ resource }) {
    ...
    this._bay.setResource(resource);
    this._stateMachine.toProvisioned();
    this._notify(XPoolEvents.RESOURCE_CREATED);
  }

  #onCreateError({ error }) {
    debug(`Error provisioning ${this.name} bay`, error);
    this._notify(XPoolEvents.RESOURCE_CREATION_ERROR, { error });
  ...
  #onCreateTimeout({ timeout }) {
    ...
    this._notify(XPoolEvents.RESOURCE_CREATION_TIMEOUT, { timeout });
    this.#segregate();
  }
